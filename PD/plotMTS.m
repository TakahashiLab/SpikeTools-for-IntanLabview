...
function [out, xl, indPyr, thetaIdx, sp, peakCoh, peakFreq,Rc] = coreMTS(Cs, fs, celltype, sm, xlabel, confCs, topFreq, div)
    ...
    Frc=diag(corr(CohAS,CohDS,'type','pearson'));%frequency
    Trc=diag(corr(CohAS,CohDS(end:-1:1,:),'type','spearman'));%time
    ...
    out = tmp;
    startT = find(xlabel == 2);
    endT = find(xlabel == 12);
    thetaIdx = xl(startT):xl(endT);

    startA = find(xlabel == 14);
    ...
    Coh=Coh(allIdx,:);
    Ctrl=Ctrl(allIdx,:);
    [clusters, p] = permutest(Coh, Ctrl, false, alpha, 1000, false);
