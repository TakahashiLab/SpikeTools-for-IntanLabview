...
% report_magnet_stats  v5.4 (完全版)
%  - 条件別の全体ローズ図（平均ベクトル表示可）→ サブプロット
%  - 個体別ローズ図（平均ベクトル＋Rayleigh検定）→ サブプロット、アスタリスク付与
%  - 個体平均方向（fish-means）のローズ図 → サブプロット、across-fish Rayleighで*付与
%  - fish-means の円統計 ペア比較（ΔθのRayleigh）を実装し、多重比較補正（Holm/FDR）
%    → OUT.circPair & summary.txt に記載。sigは*で表示。
%  - 統計用 paired raincloud（密度 + raw dots + paired-lines + median）
%    → メトリクス毎にペアtの多重補正、*注記
%  - PNGはデフォルト出力OFF（個別フラグでON可能）
%  - 既知の string×logical 乗算バグを if 分岐で解消
%
% 例は本文冒頭参照
...
% PNG 出力制御（デフォルトすべて false）
addParameter(p, 'writePNGRose',      false, @(x)islogical(x)&&isscalar(x));
addParameter(p, 'writePNGPerFish',   false, @(x)islogical(x)&&isscalar(x));
addParameter(p, 'writePNGFishMeans', false, @(x)islogical(x)&&isscalar(x));

% 方向性（非一様性）検定：Rayleigh（個体平均方向＝全体）
...
if ~isempty(opt.winlist)
    ...
    OUT = struct();
    OUT.conditions = string(conditions);
    OUT.winlist    = opt.winlist;
    OUT.results    = cell(size(opt.winlist,1),1);
...
% ★ fish-means の円統計ペア比較（ΔθのRayleigh）＋多重補正
circPair = fishmeans_circ_pairwise_tests(S, conditions_in, opt.adjust);

files.circPairwise = plot_circ_pairwise_overview( ...
    S, conditions_in, circPair, outdir, ws, wl, opt.alpha, ...
    [], false, opt.pdfRasterRes);  % tiles=[], PNG出力false
...
files.rose_png = fullfile(outdir,'rose_headingMag.png');
draw_rose_pdf_all(S, conditions_in, files.rose_pdf, files.rose_png, ...
    ws, wl, opt.pdfRasterRes, opt.drawMeanVector, opt.drawTargetOnRose, opt.targetDirDeg, opt.roseBinN, opt.writePNGRose);
...
if opt.doPerFishPlots || opt.doPerFishRayleigh
    [perFishRayleighTbl, perFishFiles] = per_fish_rose_and_tests( ...
        S, conditions_in, outdir, ws, wl, opt, opt.writePNGPerFish);
end
files.perFish = perFishFiles;

% ★ 個体平均方向のローズ図（任意）→ 1つの図(複数ページ可)に複数サブプロット
fishMeansFiles = struct();
if opt.doFishMeansPlots
    fishMeansFiles = draw_fish_means_rose_multi(S, conditions_in, outdir, ws, wl, ...
        opt.pdfRasterRes, opt.drawMeanVector, uniformTbl, opt.alpha, opt.fishMeansTiles, opt.writePNGFishMeans, circPair);
end
files.fishMeans = fishMeansFiles;

% ★ 統計検定データの可視化：paired raincloud（密度+raw dots+paired lines+median）＋有意*注記
statRainFiles = struct();
if opt.doStatsRaincloud
    statRainFiles = draw_paired_raincloud_allmetrics(PF, conditions_in, commonPairs, METRICS, outdir, ws, wl, opt.rainTiles, opt.rainWritePNG, opt.pdfRasterRes, opt.adjust, opt.alpha);
end
...
disp('--- Fish-means circular pairwise (Δθ Rayleigh) ---'); if ~isEmptyTable(circPair), disp(circPair); end
...
%% ===== 個体別ローズ + Rayleigh =====
function [rayTbl, fileMap] = per_fish_rose_and_tests(S, conditions_in, outdir, ws, wl, opt, writePNG)
% 出力：各条件ごとに per-fish ローズをページ分割して PDF/PNG（任意）で保存
%       Rayleigh 結果（個体×条件）テーブル
fileMap = struct();
...
for j = 1:numel(conditions_in)
    ...
    for k = 1:numel(pairs)
        if mod(k-1, rows*cols)==0
            if exist('f','var'), finalize_page(f, pdfPath, pngPath, opt.pdfRasterRes, pageCount>0, writePNG); close(f); end
        ...
        if opt.doPerFishRayleigh
            [pval, z, Rbar, mu_deg, n] = rayleigh_on_angles(ang);
            ...
            rayMean(end+1,1) = mu_deg;             %#ok<AGROW>
    ...
    if exist('f','var')
        finalize_page(f, pdfPath, pngPath, opt.pdfRasterRes, pageCount>0, writePNG); close(f);
        clear f;
    end

    pngOut = '';
    if writePNG, pngOut = pngPath; end
    fileMap.(safe_field(cj)) = struct('pdf', pdfPath, 'png', pngOut);
end
...
function fileMap = draw_fish_means_rose_multi(S, conditions_in, outdir, ws, wl, pngRes, drawMeanVector, uniformTbl, alpha, tiles, writePNG, circPair)
...
for j = 1:N
    ...
    % Across-fish Rayleigh p
    pv = NaN;
    if ~isempty(uniformTbl)
        ii = find(uniformTbl.Condition==string(cj),1);
        if ~isempty(ii), pv = uniformTbl.p(ii); end
    end
    ttl = sprintf('Fish-means: %s (Rayleigh p=%.3g)', safe_text(cj), pv);

    % ★ ペア比較で有意な条件ペアに * を注記（この条件に関わる有意ペアを数だけ） 
    % circPair は補正済み。該当条件が関与し、p_adj<alpha の本数だけ'*'
    nStars = 0;
    if ~isempty(circPair)
        hit = (circPair.CondA==string(cj) | circPair.CondB==string(cj)) & (circPair.p_adj < alpha);
        nStars = sum(hit);
    end
    if nStars>0, ttl = [ttl, repmat('*',1,nStars)]; end
...
pngOut = '';
if writePNG, pngOut = pngPath; end
fileMap.all = struct('pdf', pdfPath, 'png', pngOut);
end

%% ===== 統計用：paired raincloud（密度 + raw dots + paired-lines + median） =====
function fileMap = draw_paired_raincloud_allmetrics(PF, conditions_in, commonPairs, METRICS, outdir, ws, wl, tiles, writePNG, pngRes, adjustMethod, alpha)
fileMap = struct();

% タイル設定
if isempty(tiles)
    cols = 2; rows = ceil(numel(METRICS)/cols);
else
...
for mi = 1:numel(METRICS)
    ...
    % raincloud: 各条件ごとにカーネル密度 + 点 + 中央値 + ペア線
    xPositions = 1:nC;     % x 軸：条件
    maxW = 0.35;           % バイオリンの最大半幅
    yAll = X(:);
    yAll = yAll(~isnan(yAll));
    if isempty(yAll)
        title(ax, metric + " (No data)", 'Interpreter','none');
        continue;
    end
    yMin = min(yAll); yMax = max(yAll);
    yRange = yMax - yMin; if yRange<=0, yRange = 1; end
    yGrid = linspace(yMin - 0.05*yRange, yMax + 0.05*yRange, 200);

    % まず raw dots のペア線（スパゲッティ）
    for iF = 1:nF
        yi = X(iF, :);
        good = ~isnan(yi);
        xi = xPositions(good);
    ...
    for cj = 1:nC
        ...
        if all(~isfinite(fhat)) || max(fhat)<=0
            fhat = ones(size(yGrid));
            yvals = yGrid;
        end
        fhat = fhat / max(fhat); % 正規化（最大を1）
        w = fhat * maxW;

        % フルバイオリン
        xRight = cj + w; xLeft = cj - w;
        patch(ax, [xLeft, fliplr(xRight)], [yvals, fliplr(yvals)], [0.7 0.7 0.7], ...
              'FaceAlpha', 0.25, 'EdgeColor', 'none');

        % raw dots（水平方向にジッタ）
        yiRaw = X(:, cj); yiRaw = yiRaw(~isnan(yiRaw));
        xj = cj + (rand(numel(yiRaw),1)-0.5)*2*jitter;
        plot(ax, xj, yiRaw, 'o', 'MarkerSize', 4, 'LineWidth', 0.5);

        % median
        ymed = median(yiRaw, 'omitnan');
        plot(ax, [cj-0.25 cj+0.25], [ymed ymed], '-', 'LineWidth', 2.5);
    end

    % ★ ペア比較（条件間）の有意性：メトリクス毎に全ペアで paired t、補正後 p_adj<alpha に*注記
    % ペアの線+*（上部）
    if nC>=2
        % t検定（paired） across fish
        pairs = nchoosek(1:nC,2);
        pvals = nan(size(pairs,1),1);
        for ii=1:size(pairs,1)
            a = X(:,pairs(ii,1)); b = X(:,pairs(ii,2));
            v = ~isnan(a) & ~isnan(b);
            if any(v)
                [~,p] = ttest(a(v), b(v));
                pvals(ii) = p;
        ...
        % 補正
        if any(isfinite(pvals))
            switch lower(string(adjustMethod))
                case "holm", p_adj = holm_adjust(pvals(isfinite(pvals)));
                case "fdr",  p_adj = mafdr(pvals(isfinite(pvals)),'BHFDR',true);
            end
            padj_full = nan(size(pvals)); padj_full(isfinite(pvals)) = p_adj;
            % 描画（上に積み上げ）
            yTop = max(yAll) + 0.08*(yRange+eps);
            stack = zeros(1,nC); % 各位置の段数
            for ii=1:size(pairs,1)
                if ~isfinite(padj_full(ii)), continue; end
                i1 = pairs(ii,1); i2 = pairs(ii,2);
                lvl = max(stack(i1), stack(i2)) + 1;
                stack(i1) = lvl; stack(i2) = lvl;
                yBar = yTop + 0.08*yRange*(lvl-1);
                plot(ax, [i1 i1 i2 i2], [yBar-0.01*yRange yBar yBar yBar-0.01*yRange], '-', 'LineWidth', 1.0);
                if padj_full(ii) < alpha
                    text(ax, mean([i1 i2]), yBar+0.02*yRange, '*', 'HorizontalAlignment','center', 'FontSize', 14, 'FontWeight','bold');
                end
            end
        end
    end
    ...
    xlim(ax, [0.5 nC+0.5]);
    xticks(ax, xPositions); xticklabels(ax, cellstr(C));
    ax.XTickLabelRotation = 15; grid(ax,'on');
...
pngOut = '';
if writePNG, pngOut = pngPath; end
fileMap.pdf = pdfPath; fileMap.png = pngOut;
end
%% ===== Δθ 可視化：ペアごとのレインクラウド + 生点 + メディアン + コンパス（インセット） =====
function fileMap = plot_circ_pairwise_overview(S, conditions_in, circPair, outdir, ws, wl, alpha, tiles, writePNG, pngRes)
% S: sample行テーブル（Condition, Pair, cosMag, sinMag を含む）
% circPair: fishmeans_circ_pairwise_tests の結果（CondA,CondB,p_adj 等）
% tiles: [rows cols]（省略可）。未指定なら横2列に自動整列
% writePNG: false で PNG 出力しない（既定 false）
if nargin<8 || isempty(tiles)
    nPairs = size(nchoosek(1:numel(conditions_in),2),1);
    cols = min(2, nPairs); rows = ceil(nPairs/cols);
else
    rows = tiles(1); cols = tiles(2);
end
if nargin<9, writePNG = false; end
if nargin<10, pngRes = 300; end
...
% Condition を string 化
condCol = string(S.Condition);
% fish 識別があれば維持（なければ Cohort×Fish を作る）
...
tlo = tiledlayout(rows, cols, 'Padding','compact','TileSpacing','compact');
sg = sprintf('Circular pairwise Δθ (deg) | Window %gs-%gs', ws, finite_end(ws, wl));
safe_sgtitle(tlo, sg);
...
for ti = 1:nTiles
    ...
    % 角度差を度に
    ddeg = rad2deg(d);
    % レインクラウド（バイオリン風）+ 生点 + メディアン
    draw_single_raincloud(ax, ddeg);

    % circPair から p_adj を拾ってアスタリスク
    star = '';
    if ~isempty(circPair)
    ...
    % 右上にコンパス（インセット）
    inset = axes('Position', inset_rect(ax, 0.70, 0.60, 0.25, 0.35), 'Color','none', 'PolarAxes', true); %#ok<CPROPLC>
    delete(inset); % 上行は PolarAxes 指定のための誘導。実際の polaraxes を作る
    pax = polaraxes('Position', inset_rect(ax, 0.70, 0.60, 0.25, 0.35)); hold(pax,'on');
    % 平均ベクトル
    C = mean(cos(d)); Ssin = mean(sin(d)); mu = atan2(Ssin, C); Rbar = hypot(C,Ssin);
...
%% --- Δθ（rad）を作るヘルパ：条件 cA と cB の fish 平均角の差を [-pi,pi] に折りたたみ
...
% 親座標系(ax)を [0,1] として相対配置
op = get(ax,'Position'); r = [op(1)+op(3)*x, op(2)+op(4)*y, op(3)*w, op(4)*h];
end

% function s = safe_text(x)
% if isstring(x), x = x(1); end
% if iscell(x),   x = x{1}; end
% if isnumeric(x), x = num2str(x); end
% s = char(x); s = strrep(s, char(8211), '-'); s = strrep(s, char(8212), '-');
% end
function x = finite_end(ws, wl), if isfinite(wl), x = ws+wl; else, x = Inf; end, end
function out = ternary(tf,a,b), if tf, out=a; else, out=b; end, end

%% ===== Fish-means の円統計ペア比較（Δθ Rayleigh）＋多重補正 =====
function circPair = fishmeans_circ_pairwise_tests(S, conditions_in, adjustMethod)
% 各条件で「個体平均角」を fish ごとに求め、条件間の角度差 Δθ を [-pi,pi] に折りたたみ
% 0 方向への集中を Rayleigh 検定 → p を多重補正（Holm/FDR）
...
if ~istable(S)
    try
        ...
        error('fishmeans_circ_pairwise_tests: S は table または struct(table化可能) である必要があります。');
    end
end
assert(ismember('Condition', S.Properties.VariableNames), 'S に Condition 列がありません。');
assert(ismember('cosMag',    S.Properties.VariableNames), 'S に cosMag 列がありません。');
assert(ismember('sinMag',    S.Properties.VariableNames), 'S に sinMag 列がありません。');
% Condition を string に正規化
condCol = string(S.Condition);

conds = string(conditions_in(:));
pairs = nchoosek(conds,2);

CondA = strings(0,1); CondB = strings(0,1);
N = []; Z = []; P = []; MeanDeltaDeg = []; Rbar = [];
...
for r=1:size(pairs,1)
    ...
    SjA = S(condCol==cA, :);
    SjB = S(condCol==cB, :);
    if isempty(SjA) || isempty(SjB), continue; end

    % fishごとの平均角（cos/sin平均→atan2）
    [Pa, muA] = fish_mean_angles(SjA);
    [Pb, muB] = fish_mean_angles(SjB);

    % 共通 fish
    [Pab, ia, ib] = intersect(Pa, Pb, 'stable'); %#ok<ASGLU>
    if isempty(Pab), continue; end

    d = local_wrapToPi(muA(ia) - muB(ib));   % Δθ
    d = d(~isnan(d));
    ...
    CondA(end+1,1)=cA;          %#ok<AGROW>
    CondB(end+1,1)=cB;          %#ok<AGROW>
    N(end+1,1)   = n;           %#ok<AGROW>
    Z(end+1,1)   = z;           %#ok<AGROW>
    P(end+1,1)   = p;           %#ok<AGROW>
    MeanDeltaDeg(end+1,1) = rad2deg(atan2(Ssin,C)); %#ok<AGROW>
    Rbar(end+1,1) = R;         %#ok<AGROW>
end

if isempty(P)
    circPair = table(CondA,CondB);
    circPair.N=[]; circPair.Z=[]; circPair.p=[]; circPair.p_adj=[];
    circPair.MeanDeltaDeg=[]; circPair.Rbar=[];
    return;
end
...
circPair = table(CondA, CondB, N(:), Z(:), P(:), p_adj(:), MeanDeltaDeg(:), Rbar(:), ...
    'VariableNames', {'CondA','CondB','N','Z','p','p_adj','MeanDeltaDeg','Rbar'});
...
function [Pairs, mu] = fish_mean_angles(Ssub)
Ssub.Pair = strcat("C", string(Ssub.Cohort), "_F", string(Ssub.Fish));
G = findgroups(Ssub.Pair);
Pairs = splitapply(@(x) x(1), Ssub.Pair, G);
cosm = splitapply(@(x) mean(x,'omitnan'), Ssub.cosMag, G);
sinm = splitapply(@(x) mean(x,'omitnan'), Ssub.sinMag, G);
ok = ~isnan(cosm) & ~isnan(sinm);
Pairs = Pairs(ok);
...
% ★ Fish-means circular pairwise (Rayleigh on Δθ)
if ~isEmptyTable(circPair)
    sig = circPair(circPair.p_adj < alpha, :);
    if ~isempty(sig)
        lines{end+1} = 'Significant (Fish-means circular pairwise, Rayleigh on Δθ):';
        for i=1:height(sig)
            lines{end+1} = sprintf('  %s vs %s: meanΔ=%.1f°, R̄=%.3f, Z=%.2f, p=%.3g, p_adj=%.3g (N=%d)', ...
                sig.CondA(i), sig.CondB(i), sig.MeanDeltaDeg(i), sig.Rbar(i), sig.Z(i), sig.p(i), sig.p_adj(i), sig.N(i));
        end
    end
end
