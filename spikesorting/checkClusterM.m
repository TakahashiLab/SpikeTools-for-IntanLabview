function checkClusterM(ensemble,an,en,tetNum)
tm=tetrodeMap(an,en);
...
c={'r.','g.','b.','k.','m.','y.','r*','g*','k*','y*'};

%c=c([4 3 6]);%for #repeat-5
...
for i=1:size(comb,1)
    ...
    for k=1:loop            
        if any(ind==k)
            plot(Fets{k}(:,comb(i,1)),Fets{k}(:,comb(i,2)),c{cn});
            cn=cn+1;
        else
            plot(Fets{k}(:,comb(i,1)),Fets{k}(:,comb(i,2)),'.','Color',[0.5,0.5,0.5]);
        end
    end
end
