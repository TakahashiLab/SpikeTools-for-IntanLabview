function theta_phase=thetaPhase(signal,fs,method)
if nargin==2
    method='wavelet';
end
...
fprintf('calculating the theta phase\n');
switch lower(method)
  case('hilbert')
    theta_phase=InstPhase(sig_theta);
    theta_phase = mod(theta_phase,2*pi);%covert to 0-2pi rather than -pi:pi
  case('wavelet')
    samplingRate=1000;
    passband(1)=4;
    passband(2)=11;
    ...
    theta_phase = mod(lfpphases,2*pi);%covert to 0-2pi rather than -pi:pi            
end         
